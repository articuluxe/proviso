...
;; Version: 1.0
;; Modified Time-stamp: <2020-01-20 08:57:39 Dan.Harms>
...
:initfun (lambda (proj)
   (proviso-put proj :proj-alist
      ...
      (should (string= (proviso-get proviso-local-proj :project-name)
                       "c2"))
      (should (equal (proviso-get proviso-local-proj :tags-alist)
                     (list (list (concat "^\\(.*\\)" base-test-dir "a/b/c2/" "\\(.*\\)$")
                                 (concat base-test-dir "a/b/c2/.tags/base-tags")
                                 (concat base-test-dir "a/b/c2/.tags/subdir-tags"))
                                   )))
